 ...
 *                 {@code "jev-1.13.0"}); only {@link RequestModel.Pinned#id()} is populated from
 *                 the wire — {@code description}/{@code releaseDate} are {@code null} unless
 *                 separately looked up via {@link TypesafeClient#listModels()}
 ...
 */
public record EvaluateResponse(RequestModel.Pinned model, Map<String, Answer> answers, Usage usage, Metadata metadata) {
